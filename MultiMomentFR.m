...
L.lcoef = double(subs( l.lagrangePolynomial,-1)); % e.g.:[0 0 0 1]
L.rcoef = double(subs( l.lagrangePolynomial,+1)); % e.g.:[1 0 0 0]
L.dcoef = double(subs(l.dlagrangePolynomial,xi));
...
while t < tEnd
    ...
    % Plot u
    %if rem(it,10)==0, plot(x,u,x,u0,'-+',xc,u_bar,'ro'); 
...
legend(h,'Exact','MM-FR','Cell Averages'); grid on;
title('MM-FR','interpreter','latex','FontSize',18);
